            ...
            <div className="space-y-8">
               ...
               <Card className="p-6">
                  ...
                  <div className="space-y-2 max-h-48 overflow-y-auto pr-2 custom-scrollbar">
                     {disciplineData.map((item: any, idx: number) => (
                        <div key={item.name} className="flex justify-between items-center text-xs">
                           <div className="flex items-center gap-2">
                              <div className="w-2 h-2 rounded-full" style={{ backgroundColor: ['#ef4444', '#f97316', '#eab308', '#10b981', '#06b6d4', '#3b82f6', '#6366f1', '#8b5cf6'][idx % 8] }}></div>
                              <span className="text-slate-600 truncate max-w-[120px]">{item.name}</span>
                           </div>
                           <span className="font-bold text-slate-700">{((item.value / grandTotal) * 100).toFixed(1)}%</span>
                        </div>
                     ))}
                  </div>
               </Card>
